...
var current_token := ""
const PROJECT_ID := "worldsoftware-9e78c"

const FIRESTORE_URL := "https://firestore.googleapis.com/v1/projects/%s/databases/(default)/documents/" % PROJECT_ID
var user_info := {}

const VERIFY_URL := "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=%s" % API_KEY
...
func check_verify(token: String, http: HTTPRequest) -> void:
	var body := {
		"idToken": token,
	}
	http.request(VERIFY_URL, [], false, HTTPClient.METHOD_POST, to_json(body))
	var result := yield(http, "request_completed") as Array	
	var result_body := JSON.parse(result[3].get_string_from_ascii()).result as Dictionary
	#print(str(result_body))
	if "False" in str(result_body):
		isVerified = false
	else:
		isVerified = true
